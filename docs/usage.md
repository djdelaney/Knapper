...
| `ReadOnly` | false | Read-only deployment: removes (from list AND call) every tool whose `[McpServerTool]` does not declare `ReadOnly = true` — today the seven mutation tools. DERIVED from the attributes, so a write tool added later is covered automatically, and a tool that forgets the flag counts as a writer. Combines with `DisabledTools`. Verify such a deployment with `knapper verify --expect-read-only`: it then asserts exactly the read-only surface (`ToolNames.ReadOnly`) and that the write tools are UNCALLABLE, not just unlisted. Without the flag, verify fails it on MISSING tools — deliberately never inferred, or a partially registered server would pass as read-only. |
